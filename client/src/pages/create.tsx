...
import { Loader2, UserPlus, AlertCircle, Wand2, Tag, Library } from "lucide-react";
import { z } from "zod";
...
export default function CreatePersona() {
  ...
  const onSubmit = async (data: FormData) => {
    reset();
    await generate({ mode: "create", inputData: data as Record<string, unknown>, model: selectedModel, provider: selectedProvider });
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-16">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <h1 className="text-4xl md:text-5xl font-sans font-bold text-gradient mb-3">Persona Architect</h1>
        <p className="text-lg text-muted-foreground mb-8 max-w-2xl">
          Design a highly specialized AI persona. Provide the role, context, and constraints — your local model does the rest.
        </p>
      </motion.div>
      ...
      <motion.form initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}
        ...
        <div className="space-y-2">
          ...
          <textarea {...register("purpose")} data-testid="input-purpose"
            placeholder="What is this persona trying to achieve?" rows={3}
        ...
        <div className="space-y-2">
          ...
          <textarea {...register("userProfile")} data-testid="input-user-profile"
            placeholder="Who is this persona interacting with?" rows={3}
            className="w-full px-4 py-3 rounded-xl glass-input text-foreground placeholder:text-muted-foreground/50 resize-y" />
        ...
        <div className="pt-2 flex flex-wrap items-center gap-3 justify-between">
          ...
          <button type="submit" disabled={isStreaming} data-testid="button-generate-persona"
            ...
            <UserPlus className="w-5 h-5" /> Generate Persona
          </button>
        </div>
      </motion.form>
        ...
        {savedId && !isStreaming && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <button onClick={() => autoTags.mutate(savedId)} disabled={autoTags.isPending}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-500/10 text-blue-400 text-xs font-medium hover:bg-blue-500/20 transition-colors border border-blue-500/20">
              {autoTags.isPending ? <Loader2 className="w-3 h-3 animate-spin" /> : <Tag className="w-3 h-3" />} Suggest tags
            </button>
            <Link href={`/library/${savedId}`}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-secondary text-foreground text-xs font-medium hover:bg-secondary/80 transition-colors border border-border/50">
