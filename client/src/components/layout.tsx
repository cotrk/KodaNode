  ...
  return (
    <div className="min-h-screen flex bg-background text-foreground selection:bg-primary/30 overflow-hidden relative">
      ...
      <aside className={`
        ...
        w-72 glass-panel border-y-0 border-l-0 md:flex flex-col
      ...
      `}>
        <div className="p-6 flex items-center gap-3 border-b border-border/50">
          ...
          <div>
            <h1 className="font-display font-bold text-xl leading-tight text-gradient">Persona</h1>
            <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Architect</p>
          </div>
        </div>

        <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
          <div className="px-3 pb-2 text-xs font-semibold text-muted-foreground uppercase tracking-widest">
            Tools
          </div>
          {navItems.slice(0, 3).map((item) => {
            const isActive = location === item.href || (location === "/" && item.href === "/create");
            return (
          ...
          <div className="px-3 pt-6 pb-2 text-xs font-semibold text-muted-foreground uppercase tracking-widest">
            Library
          </div>
        ...
        <div className="p-4 border-t border-border/50">
          <div className="bg-secondary/50 rounded-xl p-4 border border-white/5">
            <p className="text-xs text-muted-foreground leading-relaxed">
              Design sophisticated LLM personas with guided constraints and templates.
            </p>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        <header className="md:hidden flex items-center justify-between p-4 border-b border-border/50 glass-panel border-x-0 border-t-0 z-30">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-display font-bold text-lg text-gradient">Persona Architect</h1>
          </div>
          <button 
